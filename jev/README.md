...
| `scripts/jev_diagnostics.py` | The redacted, machine-readable diagnostics report (phase 6.3): host pin, manifest and profile results, patch and adapter states, feature switches, component pins, binary digest, fixture catalog, ambient home, and isolated environment - assembled from an allow-list and re-scanned for credential shapes, with `--check` failing a lane on any finding. |
