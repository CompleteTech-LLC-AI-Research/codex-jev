...
| #8 | Validate and release the combined JEV Codex stack (phase 6) | #24–#26 | open; phase 6, #24 and #25 merged, #26 in review |
...
| #24 | Deterministic end-to-end regression scenarios (phase 6.1) | #89 | merged; issue closed |
| #25 | Isolated live-host and performance validation (phase 6.2) | #90 | merged; issue closed; the live-provider tier still needs explicit consent and a positive budget |
| #26 | Package compatibility, upgrade, and rollback workflow (phase 6.3) | #92 | in review; issue open |
...
| A missing required release member is refused where the package is built, not where it is verified. | `release_artifact.py` derives `REQUIRED_SOURCE_MEMBERS` from the same tuple `verify` checks and refuses `E_ARTIFACT_MISSING` during `build`. Without that, a checkout where a required file is still untracked produces an archive that looks built and then fails its own verifier - the failure this lane hit when the three phase 6.3 files were new - so the rule is enforced once, at the earliest point a caller can act on it. `verify` keeps the same check because it must also hold for an archive it did not build. |
| The release candidate is selected by tracking status, not by a name list. | `build` packages git-tracked paths under `jev/`, the two native modules the host compiles, and the `test_jev_*` harnesses, so `.jev/`, build output, component checkouts, and captured state are excluded structurally rather than by pattern. A tracked file that is still credential-shaped is then refused by name (`E_ARTIFACT_FORBIDDEN`) or by content (`E_ARTIFACT_CREDENTIAL`), and the one escape hatch is per-finding: the matched text itself must carry a synthetic marker, so `FIXTURE`/`EXAMPLE` cannot be sprinkled elsewhere in a file to launder a real key. `verify` recomputes the allowance set and compares it to the record, so the exemption list cannot be widened silently. |
| The release evidence proves ambient-home isolation with a control, not with the live home. | The machine's own Codex home is written by the session that produces the evidence, so it changes on its own; measuring it before and after a command window proves nothing by itself. The claim is supported instead by a negative control (`CODEX_HOME` aimed at a scratch home, identical digest before and after init/launch/rollback) and by an attribution window that compares the paths an idle 35-second window changes with the paths the commands change: the command window's set is a subset of the idle set, so nothing is attributable to the integration. |
...
| [`END_TO_END.md`](END_TO_END.md) | The composed regression harness: the composed order (capture, retrieval, screening, projection, collab, sentinel/veto, approval), the per-step artifact, the tier on every assertion, how to run it, and what it does not prove (#24). |
| [`VALIDATION.md`](VALIDATION.md) | The offline, real-host, and live-provider validation record: the baseline-vs-integrated payload bytes with a byte-derived token estimate, the observed boundary latency, the fallback and refusal counts, the host smokes, and the live-provider blocker (#25). |
| [`RELEASE.md`](RELEASE.md) | What the release package ships, the candidate artifact and its verifier, the diagnostics report, the install and upgrade order, state ownership, backups, rollback, the platform matrix, the known limitations, and the release gates G1-G9 (#26). |
| [`evidence/release-package-fresh-setup.md`](evidence/release-package-fresh-setup.md) | The fresh isolated setup behind gate G5: the candidate artifact builds, verifies against its own record and is byte-reproducible at the recorded revision, a clean checkout reproduces the `isolated-offline` plan, two hosted runs answer from the loopback fixtures, diagnostics exit 0 with no redaction findings, rollback moves the environment aside and is idempotent, and the ambient home is proved untouched by a scratch-home control plus an idle-vs-command attribution window (#26). |
...
| The merged #23 gate never binds a report to the frozen holdout it was measured on. | #23, #85 | Closed by #88 (`e6e228cc97`); filed as #85. `gate --split` now re-derives the labelled-rows digest and the holdout membership, so a report measured outside the frozen holdout is refused (`E_HOLDOUT_REQUIRED`) instead of permitted, an edited split is reported as `split_drift` with both digests, an unmeasured holdout row is reported as `holdout_incomplete`, and a caller that passes no `--split` gets `holdout.checked: false` rather than a silent upgrade to a holdout claim. Enforcement is unaffected: the shipped fixture is still not permitted to enforce. |
...
| Only `linux-x86_64` is validated for the release package. | #26 | Open; `macos-aarch64` and `windows-x86_64` are declared in the manifest and untested - no macOS or Windows host exists here - and the generated launcher is a POSIX `sh` script, so those platforms call `python3 jev/scripts/launch_isolated.py` directly. Recorded in the platform matrix in [`RELEASE.md`](RELEASE.md). |
| The candidate artifact is not signed and carries no binary. | #26 | Open; the project has one GitHub identity and no signing key on this machine, so integrity rests on the record's per-member digests, the pinned revision, `source.worktree_dirty`, and a byte-reproducible rebuild rather than on a signature. `build_provenance.py` records `binary_sha256: null` when that invocation did not build the binary. |
