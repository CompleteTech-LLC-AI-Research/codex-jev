...
| #69 | repo-checks is red on `main`: `just fmt-check` needs `ruff format` and a vendored-file exclusion | #77 | in review; CI lane, unmasked by #71 |
| #70 | Run the real host binary to show projected outgoing content and exact reset | #75 | merged; issue closed |
| #82 | repo-checks is red on `main`: prettier wants the README entry-point table realigned | #77 | merged; filed from #69 |
...
| A shadow report that counts only what it understood is not evidence. | #23 asks the report to include every deferral and failure, so the counts are derived from the lists the report prints and `verify_report()` re-derives both from the correlated cases the report carries. A case counted but not listed, a deferral missing from the rendered report, or a report with no `unmeasured` list is a failing check, and a deferred or failed case must abstain, so it can never be rounded into an agreement. |
| Enforcement is enabled by an evaluation, not by a profile. | `approval.enforcement` now requires `approval.enforcement_gate`, which requires `approval.shadow_comparison`, and the gate reads its own switch (`JEV_SWITCH_APPROVAL_ENFORCEMENT_GATE`) before it will report a would-be-enabled state. A profile that never declared the gate cannot reach enforcement through any combination of the other checks, and the shipped answer is disabled for eight separately-reported reasons. The contract the gate enforces is *declared*: `jev-codex-approval` carries an `evaluation` record (switches, report kind, opt-in action classes, consent credential, the seven Guardian-only conditions, and the promotion criteria), which `verify-manifest.py --approval-enforcement disabled` checks structurally and in state, and which the gate itself reads before it will produce any verdict - so an enforcement switch with no declared record is a missing gate in both layers, never an implicit permission. |
| Two independent implementations of #23 are consolidated into one canonical change. | A second session built #23 as `approval_shadow.py` + `APPROVAL_SHADOW.md` + a manifest `evaluation` record and validator gate (PR #81) while this branch built `shadow_comparison.py` with the frozen calibration/holdout digests, the redaction audit, the report-completeness re-derivation, and the fail-closed runtime gate (PR #83). Both wrote `.github/scripts/test_jev_shadow.py`, so landing both would have overwritten one. The comparison, freeze, redaction and gate machinery of #83 is the canonical artifact, and the load-bearing declaration half of #81 - the manifest `evaluation` record, `_validate_approval_evaluation`, `check_approval_enforcement`, and `verify-manifest.py --approval-enforcement disabled`, with their unit tests - is ported into it and asserted from the required-CI lane. #81 is closed with a cross-reference; no phase of the epic gets two implementations. |
| The split that was tuned on cannot be the split that justifies enforcement. | The calibration and holdout splits are pinned by digest in `frozen.json`, and the gate refuses a set that is not the declared holdout (`E_HOLDOUT_REQUIRED`) or criteria that name a different `evaluated_on`. Drift leaves enforcement disabled rather than erroring, because an edited evaluation set is a reason to stop, not to retry. |
...
| [`SHADOW_COMPARISON.md`](SHADOW_COMPARISON.md) | Shadow comparison and the enforcement gate: the judgment/host vocabularies and their declared mapping, correlation without raw commands, the labelled calibration/holdout splits and their frozen digests, the eight fail-closed gate checks and the Guardian-only kill switch, what the phase does not measure, and the offline-fixture evidence (#23). |
...
| `repo-checks` is red on `main` in the formatting lane: step 19 (`just fmt-check`) reformats twelve files, and step 20 (`prettier`) flags the root `README.md` table, which had not run since step 19 failed. | all | In review as #77. Filed as #69 (the `ruff format` debt, plus the `jev/scripts/jev_bus.py` `ruff.toml` exclusion it needs to stay byte-identical to the pinned component copy) and #82 (the README alignment, unmasked by #69 and also caused by a fork edit). |
