...
| #8 | Validate and release the combined JEV Codex stack (phase 6) | #24, #25, #26, #101, #104 | complete; all three sub-issues merged and closed, the composed claim landed as #101, and the executable composition over the same harnesses landed as #104 (`83d1c2fca2`) - `phase6_release.py run` reports `phase_validated=true trace=true tiers=true rollback=true readiness=true release_ready=false` over the eight declared stages. The verdict stays `release_ready: false` with `platform.matrix` `not-run`, because `macos-aarch64` and `windows-x86_64` have no current recorded run - readiness follows the evidence, not the closed sub-issues |
...
| #26 | Package compatibility, upgrade, and rollback workflow | #95 | merged; issue closed; follow-up #97 fixed by #99 and closed, follow-up #98 fixed by #106 and hardened by #112, and follow-up #111 (from the verification of the superseded #109) fixed by #113 and closed |
...
| #123 | `jev/evidence/plaintext-pinned-build.md` says there is no signing key on this machine, contradicting `ORCHESTRATION.md` and the environment | #125 | merged; issue closed - the bullet keeps its true observation (that lane's commits are unsigned and reviews are recorded self-reviews) and replaces the false reason with the clone-local signing configuration plus a cross-reference to the curated row |
...
merged-commits table below are complete through `fd195e8af0` (the merge of #137, which closes the
last row of each), and neither table carries a later merge: merges after that revision are recorded
in the prose rows under **Final synchronization** rather than appended as rows, because the tables
are scoped rather than extended. Re-fetch `main` for the live tip rather than reading any list in
this file as a pointer to it.
...
closed by #113. The table is complete through the merge named in its last row, `fd195e8af0` (the
merge of #137), and never contains a later merge; both tables share that boundary, and merges after
it are recorded in the prose rows under **Final synchronization** for the reason stated under the
Status table above.
...
| [`evidence/release-phase-claim.md`](evidence/release-phase-claim.md) | The composed phase-6 verdict: the ten gates and their evidence kinds at `4bac04bdc0`, why nine passing gates and one `not-run` platform gate make `release_ready` false while all three sub-issues are closed, the isolated round trip that reproduced the validated profile and rolled two fresh environments back with the ambient home untouched, the four mutation controls (all killed), and the two binding weaknesses the recording found - #97 and #98 - which are now fixed by #99 and #106/#112 rather than left open (#8). |
| [`PHASE6_RELEASE.md`](PHASE6_RELEASE.md) | The executable phase-6 composition behind #104: the eight stages composed from the 6.1/6.2/6.3 harnesses (lossless partition, one authority invariant per stage asserted by exact id), the tier sections that refuse an overclaimed label, the isolated round trip, and the ten-gate readiness document, with `phase_validated` deliberately kept separate from `release_ready` (#8). |
...
| The signing key is not registered as a GitHub *signing* key, so GitHub reports `unknown_key`. | all | Partially closed, and the blocker has two independent halves. Signing is achievable and is in use from the integration clone, but it is *clone-local* configuration rather than a machine setting: `gpg.format=ssh`, `commit.gpgsign=true`, and `user.signingkey=/home/agent/.ssh/id_ed25519` are set in the shared repository config at `/home/agent/jev/build-lead/codex-jev/.git/config`, which its worktrees read, while `~/.gitconfig` has no `gpg` section and no `includeIf` indirection - so a checkout without that local config does not sign by default and would produce unsigned commits while believing otherwise. The pushed commits that carry a signature really do: `git cat-file -p` shows `gpgsig -----BEGIN SSH SIGNATURE-----` for `8e08df38f6`, `82a6225e0c`, and this branch's own commits. `fe3bf9b200` was cited in this row before and is **not** one of them - it carries no `gpgsig` - so it was dropped from the list rather than the sentence weakened. GitHub reports `verified=false, reason=unknown_key` for two reasons: the private half of the account's registered *signing* key (`lighthouse WSL release signing`, id 1058580) is not on this machine, since the only private key present is the *authentication* key `session.home.complete.tech` (id 162671934) and the two public halves differ; and registering the present key for signing needs the `admin:ssh_signing_key` scope this token does not have (`gh api user/ssh_signing_keys` and the corresponding POST both return 404 with that scope message). Keys were not replaced and signing was not weakened, per the authorization. A checkout carrying a correct `gpg.ssh.allowedSignersFile` verifies the signatures locally. The resolution is either (a) install the private half of the registered signing key, or (b) obtain `admin:ssh_signing_key` and register the present key. |
...
Recorded by the integration lane after the epic was closed.
...
| Merges recorded after #121 | Every merge this file records from #121 through #141 is documentation or evidence only - none touches a path in `PINNED_INPUT_RULES` - so no pin, gate outcome or lane count here depends on which of them a reader fetches. Recorded in this file: #122 `c1474479bf` (fixing [#124](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/124)), #125 `2e8f2a8927` (fixing [#123](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/123)), #128 `615b79249b` (fixing [#126](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/126)), #130 `f678dda1b2` (fixing [#129](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/129)), #133 `f005bc7fd85e` (fixing [#132](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/132)), #137 `fd195e8af0` (fixing [#135](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/135)), #138 `5189e85327` (fixing [#134](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/134)), #139 `59ccbf6f2e` (the evidence half of [#136](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/136), re-anchoring the `linux-x86_64` record to `615b79249b`), and #141 `d1d852b563` (fixing [#140](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/140)). The anchored measurement revision and the pinned-input digest are unchanged over that span - `8bf426ff7f4c81` (52 files) at `473983ae8b`, at `615b79249b`, at `f005bc7fd85e` and at `fd195e8af0`, the revision both tables stop at - and the ten-gate result is identical at each (`release_ready=false gates=10 failed=0 not_run=1 blocking=macos-aarch64,platform.matrix,windows-x86_64`). The required lane holds as well: at `fd195e8af0`, `.github/scripts/test_jev_*.py` is **433 OK** and `jev/tests` is **183 OK**, and the relative-link sweep over all tracked markdown files still reports **0** broken links under `jev/**`. The invariant is scoped to #121..#141 because #143 - recorded in the row below - is the first merge after #121 to touch a pinned input and therefore the first to move the digest. Revisions are named rather than left as "the tip" for the same reason the anchored row above names one, and the list stops at the merges this file records, short of its own merge, rather than at the tip, for the reason stated under the Status table. |
...
| Post-close pinned-input change (#136) | The **mechanism** half of [#136](https://github.com/CompleteTech-LLC-AI-Research/codex-jev/issues/136) is fixed on branch commit `fb6d635a0` (the commit that carries the inputs; merged by the PR that added this row). It edits `jev/scripts/release_readiness.py` - itself a pinned input (`PINNED_INPUT_RULES` -> `glob jev/scripts/*.py`) - so the pinned-input digest moves from `8bf426ff7f4c81` (52 files) to `a79afab0b5` (52 files): `record_platform` now refuses to write a record whose working-tree digest differs from the digest its committed revision carries, and `classify_platform_record` refuses to credit such a record, so the pre-#116-HEAD-plus-untracked-script case can no longer produce a `verified` row (a regression test constructs it). The `linux-x86_64` matrix is re-recorded from a clean worktree of `fb6d635a0` against binary `f363d4e5` and the ten-gate result is unchanged (`release_ready=false gates=10 failed=0 not_run=1 blocking=macos-aarch64,platform.matrix,windows-x86_64`); the required lane is **434 OK** on `.github/scripts/test_jev_*.py` (the added dirty-worktree test) and **183 OK** on `jev/tests`, and the relative-link sweep over 212 tracked markdown files stays at **0** broken under `jev/**`. |

**Synchronization note.** `github` is the authoritative remote; `origin` in the
worktree clone this ledger is maintained in (`/home/agent/jev/build-lead/*`, whose
main worktree is `/home/agent/jev/build-lead/codex-jev`) is a stale local path
(`/home/agent/jev/work/codex-jev`) and was never pushed to; the same GitHub remote
...
every merge, and that cutoff is the revision both tables stop at rather than a claim about
`main` now.
