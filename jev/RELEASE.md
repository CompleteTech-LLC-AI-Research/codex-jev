...
When a record is `fail`, the gate detail quotes that record's own reason rather
than a generic wording, so the pairing error above is visible in `gates` output
and not only in the per-platform rows.
