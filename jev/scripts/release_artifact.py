...
# The record is generated by `build` itself, so the *source* members are the
# required set a checkout has to carry before an artifact that verifies can
# exist at all. Both halves check the same tuple: a missing member is refused
# while it is still a build error rather than reported later as a broken
# artifact.
REQUIRED_SOURCE_MEMBERS = tuple(
    path for path in REQUIRED_MEMBERS if path != RECORD_PATH
)
...
def build(root=None, out=None):
    ...
    absent = [path for path in REQUIRED_SOURCE_MEMBERS if path not in payloads]
    if absent:
        raise ArtifactError(
            f"E_ARTIFACT_MISSING: {', '.join(absent)} is not tracked in {root}; "
            "the candidate artifact must carry every required member, so a "
            "missing one is refused here instead of producing an archive that "
            "cannot verify"
        )
