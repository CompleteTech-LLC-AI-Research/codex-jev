...
| `real-host-binary` | [`jev/smoke/run-sentinel-hook-smoke.sh`](smoke/run-sentinel-hook-smoke.sh) + [`jev/evidence/sentinel-hook-real-host.md`](evidence/sentinel-hook-real-host.md) | A launched host on `main` runs the wired carrier: 33/33 named checks. Shadow stays observational and records three correlated incidents; enforce prevents the exact action before execution and the host quotes the component's reason; unwired records nothing under the same switches; and the coverage report says `activated: false` while three stages are wired, `true` only after a probe that the host corroborates, and `false` again after `--remove`. |

Required CI (`unittest discover -s .github/scripts -p 'test_jev_*.py'`) is 434
tests, ok; `jev/tests` is 183 tests, ok. At `fb6d635a0` the pinned-input digest
is `a79afab0b53fc9f5dfa61a42e7755725dc80956c76598157e3503db69ed28194` (52 files),
and the `linux-x86_64` platform record in `jev/evidence/platform-matrix.json` is
anchored to that revision. The vendored adapter is equal to the pinned component
on every golden case; it is a faithful translation rather than a byte-identical
copy, because the host module layout differs.
