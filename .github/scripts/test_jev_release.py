...
class GateTests(unittest.TestCase):
    ...
    def test_the_gate_refuses_a_record_whose_revision_lacks_its_digest(self):
        ...
        try:
            ...
            # The gate detail names the record and its actual reason, not the
            # generic live-provider wording that used to stand for every `fail`.
            self.assertIn("linux-x86_64", record["gate"]["detail"])
            self.assertIn(
                "do not exist at its recorded revision", record["gate"]["detail"]
            )
        finally:
            shutil.rmtree(root, ignore_errors=True)
